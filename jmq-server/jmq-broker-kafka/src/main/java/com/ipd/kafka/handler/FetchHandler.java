 ...
 */
public class FetchHandler extends AbstractHandler implements KafkaHandler {
    private final static Logger logger = LoggerFactory.getLogger(FetchHandler.class);

    private DelayedResponseManager delayedResponseManager;
    private GetMessageHandler getMessageHandler;

    public FetchHandler(GetMessageHandler getMessageHandler, DelayedResponseManager delayedResponseManager) {
        Preconditions.checkArgument(getMessageHandler != null, "GetMessageHandler can't be null");
        Preconditions.checkArgument(delayedResponseManager != null, "KafkaLongPullManager can't be null");
    ...
    public Command process(Transport transport, Command command) throws TransportException {
                ...
                try {
                    ...
                    if (e instanceof JMQException) {
                        ...
                        logger.error(String.format("Error when processing fetch request for partition [%s,%d] offset %d with correlation id %d. Possible cause: %s",
                                topic, partition, kafkaOffset, fetchRequest.getCorrelationId(), e));
        ...
        response = new Command(KafkaHeader.Builder.response(command.getHeader().getRequestId()), fetchResponse);
        if (fetchRequest.getMaxWait() <= 0 ||
                fetchRequest.getNumPartitions() <= 0 ||
                bytesReadable >= fetchRequest.getMinBytes() ||
                errorReadingData) {
            // do nothing
        } /*else if (delayedResponseManager.suspend(new DelayedResponseKey(transport, command, response, DelayedResponseKey.Type.FETCH, 5000L))) {
            // 长轮询成功
            return null;
        }*/
    ...
    @Override
    public int[] type() {
        return new int[]{KafkaCommandKeys.FETCH};
    }
}