...
public class JMQStore extends Service implements Store, StoreUnsafe {
     ...
     */
    public JMQStore(StoreConfig config, Broker broker) {
        if (config == null) {
    ...
    protected void doStart() throws Exception {
        ...
        flushManager =
                new FlushManager(config, checkPoint, journalManager, queueManager, dispatchService, this);
        // 启动服务
        eventManager.start();
        checkPoint.start();
    ...
    public PutResult putTransactionMessages(List<BrokerMessage> preMessages) throws JMQException {
        ...
        List<BrokerRefMessage> refMessages = new ArrayList<BrokerRefMessage>();

        PutResult result = new PutResult();
        result.setCode(JMQCode.CN_UNKNOWN_ERROR);
        try {
            journalManager.appendJournalLog(preMessages, result);

            for (BrokerMessage preMessage : preMessages) {
        ...
        BrokerPrepare prepare = journalManager.getBrokerPrepare(preMessages.get(0).getTxId());
        if (prepare == null) {
            throw new JMQException("txid is not exists:" + preMessages.get(0).getTxId(), JMQCode.CN_UNKNOWN_ERROR.getCode());
        }
    ...
    public PutResult commitTransaction(BrokerCommit commit) throws JMQException {
        ...
        try {
            ...
            for (BrokerRefMessage refMessage : tx.getPrepare().getMessages()) {
                PutResult curResult = new PutResult();
                curResult.setCode(JMQCode.CN_UNKNOWN_ERROR);
                journalManager.appendJournalLog(refMessage, curResult);
            ...
            journalManager.appendJournalLog(commit, result);
            committed = true;
        } finally {
            ...
            for (QueueItem item : qis) {
                ...
                if (maxFrameSize > 0 && dataSize > maxFrameSize) {
                    ...
                    if (i > 0) {
                        ...
                        logger.warn(String.format("the message dataSize %d extend maxFrameSize %d", dataSize,
                                maxFrameSize));
                    }
                }
                buffers.add(refBuf);
    ...
    public RByteBuffer readMessageBlock(long offset, int size) throws Exception {
        //maxOffset - offset maybe extend Integer.maxValue
