 ...
 * 主题偏移量
 */
public class TopicOffset implements Cloneable {

    // 主题
    private transient String topic;
    // 队列偏移量
    private ConcurrentMap<Integer, QueueOffset> offsets = new ConcurrentHashMap<Integer, QueueOffset>();
    // 消费者订阅时间
    ...
    public ConcurrentMap<Integer, QueueOffset> getOffsets() {
        return offsets;
    }

    public void setOffsets(ConcurrentMap<Integer, QueueOffset> offsets) {
     ...
     * @param queueId
     */
    public QueueOffset getOffset(final int queueId) {
     ...
     * @return 队列偏移量
     */
    public QueueOffset getAndCreateOffset(final int queueId) {
     ...
     * @param offset   位置
     */
    public void resetAckOffset(final int queueId, final String consumer, final long offset) {
    ...
    public void updateOffset(final TopicOffset topicOffset) {
        ...
        for (Map.Entry<Integer, QueueOffset> entry : topicOffset.offsets.entrySet()) {
    ...
    public void addConsumer(String consumer) {
        consumers.putIfAbsent(consumer, SystemClock.getInstance().now());
    }
    ...
    @Override
    public TopicOffset clone() throws CloneNotSupportedException {
        TopicOffset copyOfTopicOffset = (TopicOffset) super.clone();
        for (Map.Entry<Integer, QueueOffset> entry : copyOfTopicOffset.offsets.entrySet()) {
            copyOfTopicOffset.offsets.put(entry.getKey(), entry.getValue().clone());
        }
        return copyOfTopicOffset;
    }
}
