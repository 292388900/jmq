 ...
 */
public class RequestBarrier<C extends Config> {
    ...
    public RequestBarrier(C config) {
        this.config = config;
    ...
    public void evict() {
        ...
        while (it.hasNext()) {
            ...
            timeout = future.getBeginTime() + future.getTimeout() + 1000;
                ...
                if (future.release()) {
                    try {
                        future.onFailed(TransportException.RequestTimeoutException
                                .build(Ipv4.toAddress(future.getTransport().remoteAddress())));
                    } catch (Throwable e) {
                        logger.error("clear timeout response exception", e);
                    }
                    logger.info(String.format("remove timeout request id=%d begin=%d timeout=%d", future.getRequestId(),
     ...
     */
    public void acquire(final SemaphoreType type, final int timeout) throws TransportException {
        if (type == null) {
            return;
        }
